...
export const DetailsActionsSocial = () => {
  const classes = useStyles();
  const [copyStatus, copy] = useCopyToClipboard(window.location.href);
  return (
    <Box className={classes.actions}>
      <ShareButton />
      <Button onClick={copy} variant="outlined" className={classes.actionButton}>
        {copyStatus === "copied" ? "Copied" : <SvgIcon viewBox="0 0 18 18" color="primary" component={LinkIcon} />}
      </Button>
    </Box>
  );
};
